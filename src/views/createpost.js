...
const CreatePost = () => {
  ...
  const handleUploadPhoto = async () => {
    const images = await uploadImageToFirebase(
      imageAsset,
      `${FIREBASE_CONFIG.IMAGES_STORAGE}`,
    );
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView}>
        <View>
          <View flex row style={styles.infoBar}>
            <View flex row>
              <Avatar
                ...
                size={60}></Avatar>
              <View marginL-10>
                <Text style={styles.userName}>Nguyễn Van A</Text>
          ...
          {!isVideo && (
            <ListImageComponent listImage={imageAsset} isVideo={false} />
          )}
          {isVideo && (
            <ListImageComponent video={imageAsset[0]} isVideo={true} />
          )}
        </View>
      </ScrollView>
      <Button
