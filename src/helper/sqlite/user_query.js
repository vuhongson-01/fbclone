...
export async function insertUser(user) {
  let query =
    'INSERT INTO users (_id, phonenumber, username, password, token, last_access) VALUES (?, ?, ?, ?, ?, ?)';
...
export async function updateUser(data) {
  let query =
    'UPDATE users SET phonenumber = ?, username = ?, password = ?, token = ?, last_access = ? WHERE _id = ?';
